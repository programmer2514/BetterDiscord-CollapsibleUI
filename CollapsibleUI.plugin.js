...
`The library plugin needed for ${config.info.name} is missing. Please click Download Now to install it.`, {
  ...
  return class CollapsibleUI extends Plugin {
    ...
    start = () => {
      ...
      Library.DiscordModules.Dispatcher.subscribe('POST_CONNECTION_OPEN', this.initialize);
    ...
    getSettingsPanel = () => {
      ...
      var settingKBProfilePanel = new zps.Textbox('Toggle User Profile - Shortcut',
          null,
          BdApi.getData('CollapsibleUI', 'keyStringList').split(',')[this.I_USER_PROFILE],
      ...
      var settingDUDistCMembersList = new zps.Textbox('Members List - Closing Distance (px)',
          ...
          null, { placeholder: 'Default: 30' });
      var settingDUDistProfilePanel = new zps.Textbox('User Profile - Opening Distance (px)',
          'Distance that mouse must be from element in order for it to expand',
          BdApi.getData('CollapsibleUI', 'dynamicUncollapseDistance').split(',')[this.I_USER_PROFILE],
          null, { placeholder: 'Default: 30' });
      var settingDUDistCProfilePanel = new zps.Textbox('User Profile - Closing Distance (px)',
          'Distance that mouse must be from element in order for it to collapse',
          BdApi.getData('CollapsibleUI', 'dynamicUncollapseCloseDistance').split(',')[this.I_USER_PROFILE],
          null, { placeholder: 'Default: 30' });
      ...
      var settingDUProfilePanel = new zps.Switch('User Profile',
          ...
          BdApi.getData('CollapsibleUI', 'dynamicUncollapseEnabled').split(',')[this.I_USER_PROFILE] === 'true');
      ...
      var settingACProfilePanel = new zps.Textbox('User Profile - Threshold',
          'Maximum width for element to remain uncollapsed',
          BdApi.getData('CollapsibleUI', 'autoCollapseThreshold').split(',')[this.I_USER_PROFILE],
      ...
      var settingCAProfilePanel = new zps.Textbox('User Profile',
          null,
          BdApi.getData('CollapsibleUI', 'autoCollapseConditionals').split(',')[this.I_USER_PROFILE],
          null, { placeholder: 'Default: <blank>' });
      ...
      var settingProfilePanel = new zps.Slider('User Profile',
          '[Default = 8, Disabled = 0] - Sets order index of the User Profile button (right panel in DMs)',
          0,
          8,
          BdApi.getData('CollapsibleUI', 'buttonsOrder').split(',').map(Number)[this.I_USER_PROFILE],
          null, {
          markers: [0, 1, 2, 3, 4, 5, 6, 7, 8],
          stickToMarkers: true,
          equidistant: true
      ...
      settingTransitionSpeed.onChange = function (result) { BdApi.setData('CollapsibleUI', 'transitionSpeed', result); };
      ...
      settingCollapsedDistance.onChange = function (result) { BdApi.setData('CollapsibleUI', 'collapsedDistance', result); };
      settingButtonCollapseFudgeFactor.onChange = function (result) { BdApi.setData('CollapsibleUI', 'buttonCollapseFudgeFactor', result); };
      settingDynamicUncollapseDelay.onChange = function (result) { BdApi.setData('CollapsibleUI', 'dynamicUncollapseDelay', result); };
      ...
      settingSettingsButtonsMaxWidth.onChange = function (result) { BdApi.setData('CollapsibleUI', 'settingsButtonsMaxWidth', result); };
      settingMessageBarButtonsMaxWidth.onChange = function (result) { BdApi.setData('CollapsibleUI', 'messageBarButtonsMaxWidth', result); };
      settingMessageBarButtonsMinWidth.onChange = function (result) { BdApi.setData('CollapsibleUI', 'messageBarButtonsMinWidth', result); };
      settingToolbarIconMaxWidth.onChange = function (result) { BdApi.setData('CollapsibleUI', 'toolbarIconMaxWidth', result); };
      settingMembersListMaxWidth.onChange = function (result) { BdApi.setData('CollapsibleUI', 'membersListMaxWidth', result); };
      settingProfilePanelMaxWidth.onChange = function (result) { BdApi.setData('CollapsibleUI', 'profilePanelMaxWidth', result); };
      settingToolbarMaxWidth.onChange = function (result) { BdApi.setData('CollapsibleUI', 'toolbarMaxWidth', result); };
      settingUserAreaMaxHeight.onChange = function (result) { BdApi.setData('CollapsibleUI', 'userAreaMaxHeight', result); };
      settingMsgBarMaxHeight.onChange = function (result) { BdApi.setData('CollapsibleUI', 'msgBarMaxHeight', result); };
      settingWindowBarHeight.onChange = function (result) { BdApi.setData('CollapsibleUI', 'windowBarHeight', result); };
    ...
    terminate = () => {
      try {
        ...
        try { this.searchBar.previousElementSibling.style.removeProperty('display'); } catch {}
        ...
        if (this.avatarWrapper) { this.avatarWrapper.style.removeProperty('min-width'); }
    ...
    addButtonClickHandlers = () => {
      ...
      if (this.serverListButton) {
        this.serverListButton.addEventListener('click', function () { cui.toggleButton(cui.I_SERVER_LIST); }, { signal: this.eventListenerSignal });

        this.serverListButton.addEventListener('mouseenter', function () { this.tooltip = cui.createTooltip(cui.localeLabels.serverList + ` (${cui.keyStringList[cui.I_SERVER_LIST]})`, this); }, { signal: this.eventListenerSignal });

        this.serverListButton.addEventListener('mouseleave', function () { this.tooltip.remove(); }, { signal: this.eventListenerSignal });
      }

      // Add event listener to the Channel List button to update the icon, UI, & settings on click
      if (this.channelListButton) {
        this.channelListButton.addEventListener('click', function () { cui.toggleButton(cui.I_CHANNEL_LIST); }, { signal: this.eventListenerSignal });

        this.channelListButton.addEventListener('mouseenter', function () { this.tooltip = cui.createTooltip(cui.localeLabels.channelList + ` (${cui.keyStringList[cui.I_CHANNEL_LIST]})`, this); }, { signal: this.eventListenerSignal });

        this.channelListButton.addEventListener('mouseleave', function () { this.tooltip.remove(); }, { signal: this.eventListenerSignal });
      }

      // Add event listener to the Message Bar button to update the icon, UI, & settings on click
      if (this.msgBarButton) {
        this.msgBarButton.addEventListener('click', function () { cui.toggleButton(cui.I_MSG_BAR); }, { signal: this.eventListenerSignal });

        this.msgBarButton.addEventListener('mouseenter', function () { this.tooltip = cui.createTooltip(cui.localeLabels.msgBar + ` (${cui.keyStringList[cui.I_MSG_BAR]})`, this); }, { signal: this.eventListenerSignal });

        this.msgBarButton.addEventListener('mouseleave', function () { this.tooltip.remove(); }, { signal: this.eventListenerSignal });
      }

      // Add event listener to the Window Bar button to update the icon, UI, & settings on click
      if (this.windowBarButton) {
        this.windowBarButton.addEventListener('click', function () { cui.toggleButton(cui.I_WINDOW_BAR); }, { signal: this.eventListenerSignal });

        this.windowBarButton.addEventListener('mouseenter', function () { this.tooltip = cui.createTooltip(cui.localeLabels.windowBar + ` (${cui.keyStringList[cui.I_WINDOW_BAR]})`, this); }, { signal: this.eventListenerSignal });

        this.windowBarButton.addEventListener('mouseleave', function () { this.tooltip.remove(); }, { signal: this.eventListenerSignal });
      }

      // Add event listener to the Members List button to update the icon, UI, & settings on click
      if (this.membersListButton) {
        this.membersListButton.addEventListener('click', function () { cui.toggleButton(cui.I_MEMBERS_LIST); }, { signal: this.eventListenerSignal });

        this.membersListButton.addEventListener('mouseenter', function () { this.tooltip = cui.createTooltip(cui.localeLabels.membersList + ` (${cui.keyStringList[cui.I_MEMBERS_LIST]})`, this); }, { signal: this.eventListenerSignal });

        this.membersListButton.addEventListener('mouseleave', function () { this.tooltip.remove(); }, { signal: this.eventListenerSignal });
      }

      // Add event listener to the User Profile button to update the icon, UI, & settings on click
      if (this.profilePanelButton) {
        this.profilePanelButton.addEventListener('click', function () { cui.toggleButton(cui.I_USER_PROFILE); }, { signal: this.eventListenerSignal });

        this.profilePanelButton.addEventListener('mouseenter', function () { this.tooltip = cui.createTooltip(cui.localeLabels.profilePanel + ` (${cui.keyStringList[cui.I_USER_PROFILE]})`, this); }, { signal: this.eventListenerSignal });

        this.profilePanelButton.addEventListener('mouseleave', function () { this.tooltip.remove(); }, { signal: this.eventListenerSignal });
      }

      // Add event listener to the User Area button to update the icon, UI, & settings on click
      if (this.userAreaButton) {
        this.userAreaButton.addEventListener('click', function () { cui.toggleButton(cui.I_USER_AREA); }, { signal: this.eventListenerSignal });

        this.userAreaButton.addEventListener('mouseenter', function () { this.tooltip = cui.createTooltip(cui.localeLabels.userArea + ` (${cui.keyStringList[cui.I_USER_AREA]})`, this); }, { signal: this.eventListenerSignal });

        this.userAreaButton.addEventListener('mouseleave', function () { this.tooltip.remove(); }, { signal: this.eventListenerSignal });
      }

      // Add event listener to the Call Container button to update the icon, UI, & settings on click
      if (this.callContainerButton) {
        this.callContainerButton.addEventListener('click', function () { cui.toggleButton(cui.I_CALL_CONTAINER); }, { signal: this.eventListenerSignal });

        this.callContainerButton.addEventListener('mouseenter', function () { this.tooltip = cui.createTooltip(cui.localeLabels.callContainer + ` (${cui.keyStringList[cui.I_CALL_CONTAINER]})`, this); }, { signal: this.eventListenerSignal });

        this.callContainerButton.addEventListener('mouseleave', function () { this.tooltip.remove(); }, { signal: this.eventListenerSignal });
      }
    }
    ...
    addMiscEventListeners = (buttonsActive, settingsButtons, singleButtonWidth) => {
      ...
      if (this.dynamicUncollapse && !this.disableTransitions) {
        ...
        if (this.autoCollapse) {
          window.addEventListener('resize', function (event) {
            if (cui.serverListButton &&
              ...
              (((cui.isHSLLoaded ? BdApi.DOM.screenHeight : BdApi.DOM.screenWidth) < cui.autoCollapseThreshold[cui.I_SERVER_LIST] && BdApi.getData('CollapsibleUI', 'serverListButtonActive') === 'true') ||
                ((cui.isHSLLoaded ? BdApi.DOM.screenHeight : BdApi.DOM.screenWidth) > cui.autoCollapseThreshold[cui.I_SERVER_LIST] && BdApi.getData('CollapsibleUI', 'serverListButtonActive') === 'false'))) { cui.toggleButton(cui.I_SERVER_LIST); }
            if (cui.channelListButton &&
              ...
              ((BdApi.DOM.screenWidth < cui.autoCollapseThreshold[cui.I_CHANNEL_LIST] && BdApi.getData('CollapsibleUI', 'channelListButtonActive') === 'true') ||
                (BdApi.DOM.screenWidth > cui.autoCollapseThreshold[cui.I_CHANNEL_LIST] && BdApi.getData('CollapsibleUI', 'channelListButtonActive') === 'false'))) { cui.toggleButton(cui.I_CHANNEL_LIST); }
            if (cui.msgBarButton &&
              ...
              ((BdApi.DOM.screenHeight < cui.autoCollapseThreshold[cui.I_MSG_BAR] && BdApi.getData('CollapsibleUI', 'msgBarButtonActive') === 'true') ||
                (BdApi.DOM.screenHeight > cui.autoCollapseThreshold[cui.I_MSG_BAR] && BdApi.getData('CollapsibleUI', 'msgBarButtonActive') === 'false'))) { cui.toggleButton(cui.I_MSG_BAR); }
            if (cui.windowBarButton &&
              ...
              ((BdApi.DOM.screenHeight < cui.autoCollapseThreshold[cui.I_WINDOW_BAR] && BdApi.getData('CollapsibleUI', 'windowBarButtonActive') === 'true') ||
                (BdApi.DOM.screenHeight > cui.autoCollapseThreshold[cui.I_WINDOW_BAR] && BdApi.getData('CollapsibleUI', 'windowBarButtonActive') === 'false'))) { cui.toggleButton(cui.I_WINDOW_BAR); }
            if (cui.membersListButton &&
              ...
              ((BdApi.DOM.screenWidth < cui.autoCollapseThreshold[cui.I_MEMBERS_LIST] && BdApi.getData('CollapsibleUI', 'membersListButtonActive') === 'true') ||
                (BdApi.DOM.screenWidth > cui.autoCollapseThreshold[cui.I_MEMBERS_LIST] && BdApi.getData('CollapsibleUI', 'membersListButtonActive') === 'false'))) { cui.toggleButton(cui.I_MEMBERS_LIST); }
            if (cui.profilePanelButton &&
              ((cui.autoCollapseConditionals[cui.I_USER_PROFILE] === '') || !(cui.conditionalAutoCollapse)) &&
              ((BdApi.DOM.screenWidth < cui.autoCollapseThreshold[cui.I_USER_PROFILE] && BdApi.getData('CollapsibleUI', 'profilePanelButtonActive') === 'true') ||
                (BdApi.DOM.screenWidth > cui.autoCollapseThreshold[cui.I_USER_PROFILE] && BdApi.getData('CollapsibleUI', 'profilePanelButtonActive') === 'false'))) { cui.toggleButton(cui.I_USER_PROFILE); }
            if (cui.userAreaButton &&
              ...
              ((BdApi.DOM.screenHeight < cui.autoCollapseThreshold[cui.I_USER_AREA] && BdApi.getData('CollapsibleUI', 'userAreaButtonActive') === 'true') ||
                (BdApi.DOM.screenHeight > cui.autoCollapseThreshold[cui.I_USER_AREA] && BdApi.getData('CollapsibleUI', 'userAreaButtonActive') === 'false'))) { cui.toggleButton(cui.I_USER_AREA); }
            if (cui.callContainerButton &&
              ...
              ((BdApi.DOM.screenHeight < cui.autoCollapseThreshold[cui.I_CALL_CONTAINER] && BdApi.getData('CollapsibleUI', 'callContainerButtonActive') === 'true') ||
                (BdApi.DOM.screenHeight > cui.autoCollapseThreshold[cui.I_CALL_CONTAINER] && BdApi.getData('CollapsibleUI', 'callContainerButtonActive') === 'false'))) { cui.toggleButton(cui.I_CALL_CONTAINER); }
        ...
        if (this.enableFullToolbarCollapse) {
          this.toolBar.addEventListener('mouseenter', function () { this.style.maxWidth = cui.toolbarMaxWidth + 'px'; }, { signal: this.eventListenerSignal });
        }
        ...
        if (!this.disableSettingsCollapse) {
          this.settingsContainer.addEventListener('mouseenter', function () {
            for (let i = 0; i < (settingsButtons.length - 1); i++) { settingsButtons[i].style.maxWidth = cui.settingsButtonsMaxWidth + 'px'; }
          }, { signal: this.eventListenerSignal });
        }

        // Add event listeners to the Message Bar Button Container to update on hover
        if ((!this.disableMsgBarBtnCollapse) && this.msgBarBtnContainer) {
          this.msgBarBtnContainer.addEventListener('mouseenter', function () { this.style.maxWidth = cui.messageBarButtonsMaxWidth + 'px'; }, { signal: this.eventListenerSignal });
        }
      }
        ...
        window.addEventListener('keydown', function (e) {
          if ((e.ctrlKey || e.altKey || e.shiftKey) && (e.key != 'Dead')) {
            navigator.keyboard.getLayoutMap().then((kbMap) => {
              for (let i = 0; i < cui.buttonsOrder.length; i++) {
    ...
    applyAutocollapseConditionals = () => {
      if (this.conditionalAutoCollapse) {
        ...
        if ((this.autoCollapseConditionals[this.I_USER_PROFILE] !== '')
           && ((eval(this.autoCollapseConditionals[this.I_USER_PROFILE]) && (BdApi.getData('CollapsibleUI', 'profilePanelButtonActive') === 'true'))
             || (!eval(this.autoCollapseConditionals[this.I_USER_PROFILE]) && (BdApi.getData('CollapsibleUI', 'profilePanelButtonActive') === 'false'))))
    ...
    collapseToolbarIcons = (buttonsActive) => {
      ...
      } else if (this.profilePanelButton && (buttonsActive[this.I_USER_PROFILE] == Math.max.apply(Math, buttonsActive))) {
      ...
      } else { document.querySelectorAll('.collapsible-ui-element').forEach(e => e.style.display = 'none'); }
    }
    ...
    initToolbar = () => {
      ...
      try {
        if (this.inviteToolbar || this.searchBar) {
          ...
          this.toolBar.insertBefore(this.toolbarContainer, this.toolBar.childNodes[this.toolBar.childNodes.length - 2]);
    ...
    initUI = () => {
      ...
      if (this.serverList) {
        if (this.buttonsOrder[this.I_SERVER_LIST] || this.disabledButtonsStayCollapsed) {
          if (BdApi.getData('CollapsibleUI', 'serverListButtonActive') === 'false') {
            ...
            if (this.disableTransitions) { this.serverList.style.display = 'none'; } else {
            ...
            if (this.isHSLLoaded) { this.windowBase.style.setProperty('top', '0px', 'important'); }
      ...
      if (this.channelList) {
        if (this.buttonsOrder[this.I_CHANNEL_LIST] || this.disabledButtonsStayCollapsed) {
          if (BdApi.getData('CollapsibleUI', 'channelListButtonActive') === 'false') {
            ...
            if (this.disableTransitions) { this.channelList.style.display = 'none'; } else {
        ...
        if (this.buttonsOrder[this.I_MSG_BAR] || this.disabledButtonsStayCollapsed) {
          if (BdApi.getData('CollapsibleUI', 'msgBarButtonActive') === 'false') {
            ...
            if (!(document.querySelector(this.classTextInput)?.innerHTML)) {
              if (this.disableTransitions) { this.msgBar.style.display = 'none'; } else {
      ...
      if (this.windowBar) {
        if (this.buttonsOrder[this.I_WINDOW_BAR] || this.disabledButtonsStayCollapsed) {
          if (BdApi.getData('CollapsibleUI', 'windowBarButtonActive') === 'false') {
            ...
            if (this.disableTransitions) { this.windowBar.style.display = 'none'; } else {
      ...
      if (this.membersList) {
        if (this.buttonsOrder[this.I_MEMBERS_LIST] || this.disabledButtonsStayCollapsed) {
          if (BdApi.getData('CollapsibleUI', 'membersListButtonActive') === 'false') {
            ...
            if (this.disableTransitions) { this.membersList.style.display = 'none'; } else {
      ...
      if (this.profilePanel) {
        if (this.buttonsOrder[this.I_USER_PROFILE] || this.disabledButtonsStayCollapsed) {
          if (BdApi.getData('CollapsibleUI', 'profilePanelButtonActive') === 'false') {
            ...
            if (this.disableTransitions) { this.profilePanel.style.display = 'none'; } else {
      ...
      if (this.userArea) {
        if (this.buttonsOrder[this.I_USER_AREA] || this.disabledButtonsStayCollapsed) {
          if (BdApi.getData('CollapsibleUI', 'userAreaButtonActive') === 'false') {
            ...
            if (this.disableTransitions) { this.userArea.style.display = 'none'; } else { this.userArea.style.maxHeight = this.collapsedDistance + 'px'; }
      ...
      if (document.querySelector('.' + this.classCallContainer)) {
        if (this.buttonsOrder[this.I_CALL_CONTAINER] || this.disabledButtonsStayCollapsed) {
          if (BdApi.getData('CollapsibleUI', 'callContainerButtonActive') === 'false') {
            ...
            if (document.querySelector('.' + this.classCallContainer)) {
              if (this.disableTransitions) { document.querySelector('.' + this.classCallContainer).style.display = 'none'; } else {
      ...
      if (!this.disableTransitions) {
        ...
        if (this.resizableChannelList) {
          ...
          document.body.addEventListener('mousedown', function () { cui.channelList.style.transition = 'none'; }, { signal: this.eventListenerSignal });
          ...
          this.channelListWidthObserver = new MutationObserver((mutationList) => {
            try {
              if (((!cui.isCollapsed[cui.I_CHANNEL_LIST]) || (BdApi.getData('CollapsibleUI', 'channelListButtonActive') === 'true')) && !document.fullscreen) {
                ...
                if (parseInt(cui.channelList.style.width)) { cui.channelListWidth = parseInt(cui.channelList.style.width); } else if (cui.channelListWidth != 0) {
                  cui.channelList.style.transition = 'none';
                  cui.channelList.style.width = cui.channelListWidth + 'px';
                  cui.channelList.style.transition = 'width ' + cui.transitionSpeed + 'ms';
                } else { cui.channelList.style.removeProperty('width'); }
    ...
    tickDynamicUncollapse = (settingsButtons, buttonsActive, singleButtonWidth) => {
      ...
      if (!this.disableSettingsCollapse) {
        if (!this.isNear(this.settingsContainer, this.buttonCollapseFudgeFactor, this.mouseX, this.mouseY)) {
          for (let i = 0; i < (settingsButtons.length - 1); i++) { settingsButtons[i].style.maxWidth = '0px'; }
        }
      }
      ...
      if ((BdApi.getData('CollapsibleUI', 'serverListButtonActive') === 'false') && this.serverList) {
        if (this.dynamicUncollapseEnabled[this.I_SERVER_LIST] && this.isCollapsed[this.I_SERVER_LIST] && this.isNear(this.serverList, this.dynamicUncollapseDistance[this.I_SERVER_LIST], this.mouseX, this.mouseY) && !(this.isNear(this.msgBar, 0, this.mouseX, this.mouseY))) {
        ...
        } else if (!this.dynamicUncollapseEnabled[this.I_SERVER_LIST] || ((!(this.isCollapsed[this.I_SERVER_LIST]) || this.serverDUDelay) && !(this.isNear(this.serverList, this.dynamicUncollapseCloseDistance[this.I_SERVER_LIST], this.mouseX, this.mouseY)))) {
          if (this.serverDUDelay) {
            clearTimeout(this.serverDUDelay);
            this.serverDUDelay = false;
          }
      ...
      if ((BdApi.getData('CollapsibleUI', 'channelListButtonActive') === 'false') && this.channelList) {
        if (this.dynamicUncollapseEnabled[this.I_CHANNEL_LIST] && this.isCollapsed[this.I_CHANNEL_LIST] && this.isNear(this.channelList, this.dynamicUncollapseDistance[this.I_CHANNEL_LIST], this.mouseX, this.mouseY) && !(this.isNear(this.msgBar, 0, this.mouseX, this.mouseY))) {
        ...
        } else if (!this.dynamicUncollapseEnabled[this.I_CHANNEL_LIST] || (!(this.isCollapsed[this.I_CHANNEL_LIST]) && !(this.isNear(this.channelList, this.dynamicUncollapseCloseDistance[this.I_CHANNEL_LIST], this.mouseX, this.mouseY)))) {
          if (this.channelDUDelay) {
            clearTimeout(this.channelDUDelay);
            this.channelDUDelay = false;
          }
      ...
      if ((BdApi.getData('CollapsibleUI', 'msgBarButtonActive') === 'false') && this.msgBar) {
        if (this.dynamicUncollapseEnabled[this.I_MSG_BAR] && this.isCollapsed[this.I_MSG_BAR] && this.isNear(this.msgBar, this.dynamicUncollapseDistance[this.I_MSG_BAR], this.mouseX, this.mouseY)) {
      ...
      if ((BdApi.getData('CollapsibleUI', 'windowBarButtonActive') === 'false') && this.windowBar) {
        if (this.dynamicUncollapseEnabled[this.I_WINDOW_BAR] && this.isCollapsed[this.I_WINDOW_BAR] && this.isNear(this.windowBar, this.dynamicUncollapseDistance[this.I_WINDOW_BAR], this.mouseX, this.mouseY)) {
        ...
        } else if (!this.dynamicUncollapseEnabled[this.I_WINDOW_BAR] || (!(this.isCollapsed[this.I_WINDOW_BAR]) && !(this.isNear(this.windowBar, this.dynamicUncollapseCloseDistance[this.I_WINDOW_BAR], this.mouseX, this.mouseY)))) {
          if (this.windowDUDelay) {
            clearTimeout(this.windowDUDelay);
            this.windowDUDelay = false;
          }
      ...
      if ((BdApi.getData('CollapsibleUI', 'membersListButtonActive') === 'false') && this.membersList) {
        if (this.dynamicUncollapseEnabled[this.I_MEMBERS_LIST] && this.isCollapsed[this.I_MEMBERS_LIST] && this.isNear(this.membersList, this.dynamicUncollapseDistance[this.I_MEMBERS_LIST], this.mouseX, this.mouseY) && !(this.isNear(this.msgBar, 0, this.mouseX, this.mouseY))) {
        ...
        } else if (!this.dynamicUncollapseEnabled[this.I_MEMBERS_LIST] || (!(this.isCollapsed[this.I_MEMBERS_LIST]) && !(this.isNear(this.membersList, this.dynamicUncollapseCloseDistance[this.I_MEMBERS_LIST], this.mouseX, this.mouseY)) && !(this.isNear(document.querySelector('.' + this.classUserPopout), 10000, this.mouseX, this.mouseY)))) {
          if (this.membersDUDelay) {
            clearTimeout(this.membersDUDelay);
            this.membersDUDelay = false;
          }
      ...
      if ((BdApi.getData('CollapsibleUI', 'profilePanelButtonActive') === 'false') && this.profilePanel) {
        if (this.dynamicUncollapseEnabled[this.I_USER_PROFILE] && this.isCollapsed[this.I_USER_PROFILE] && this.isNear(this.profilePanel, this.dynamicUncollapseDistance[this.I_USER_PROFILE], this.mouseX, this.mouseY) && !(this.isNear(this.msgBar, 0, this.mouseX, this.mouseY))) {
        ...
        } else if (!this.dynamicUncollapseEnabled[this.I_USER_PROFILE] || (!(this.isCollapsed[this.I_USER_PROFILE]) && !(this.isNear(this.profilePanel, this.dynamicUncollapseCloseDistance[this.I_USER_PROFILE], this.mouseX, this.mouseY)) && !(this.isNear(document.querySelector('.' + this.classUserPopout), 10000, this.mouseX, this.mouseY)))) {
          if (this.panelDUDelay) {
            clearTimeout(this.panelDUDelay);
            this.panelDUDelay = false;
          }
      ...
      if ((BdApi.getData('CollapsibleUI', 'userAreaButtonActive') === 'false') && this.userArea) {
        if (this.dynamicUncollapseEnabled[this.I_USER_AREA] && this.isCollapsed[this.I_USER_AREA] && this.isNear(this.userArea, this.dynamicUncollapseDistance[this.I_USER_AREA], this.mouseX, this.mouseY)) {
          if (this.userDUDelay) {
        ...
        } else if (!this.dynamicUncollapseEnabled[this.I_USER_AREA] || (!(this.isCollapsed[this.I_USER_AREA]) && !(this.isNear(this.userArea, this.dynamicUncollapseCloseDistance[this.I_USER_AREA], this.mouseX, this.mouseY)))) {
          if (this.userDUDelay) {
            clearTimeout(this.userDUDelay);
            this.userDUDelay = false;
          }
      ...
      // Call Container
      if ((BdApi.getData('CollapsibleUI', 'callContainerButtonActive') === 'false') && document.querySelector('.' + this.classCallContainer)) {
        if (this.dynamicUncollapseEnabled[this.I_CALL_CONTAINER] && this.isCollapsed[this.I_CALL_CONTAINER] && this.isNear(document.querySelector('.' + this.classCallContainer), this.dynamicUncollapseDistance[this.I_CALL_CONTAINER], this.mouseX, this.mouseY)) {
          if (this.callDUDelay) {
        ...
        } else if (!this.dynamicUncollapseEnabled[this.I_CALL_CONTAINER] || (!(this.isCollapsed[this.I_CALL_CONTAINER]) && !(this.isNear(document.querySelector('.' + this.classCallContainer), this.dynamicUncollapseCloseDistance[this.I_CALL_CONTAINER], this.mouseX, this.mouseY)))) {
          if (this.callDUDelay) {
            clearTimeout(this.callDUDelay);
            this.callDUDelay = false;
          }
    ...
    toggleButton = (index) => {
      ...
      case 0: // I_SERVER_LIST
        if (BdApi.getData('CollapsibleUI', 'serverListButtonActive') === 'true') {
          ...
          if (this.disableTransitions) { this.serverList.style.removeProperty('display'); } else {
            this.serverList.style.removeProperty('width');
            if ((!this.isHSLLoaded) && this.isDarkMatterLoaded) {
          ...
          if (this.isHSLLoaded) { this.windowBase.style.removeProperty('top'); }
          BdApi.setData('CollapsibleUI', 'serverListButtonActive', 'true');
          this.serverListButton.classList.add(this.classSelected);
        }
        break;
      ...
      case 1: // I_CHANNEL_LIST
        if (BdApi.getData('CollapsibleUI', 'channelListButtonActive') === 'true') {
          ...
          if (this.disableTransitions) { this.channelList.style.removeProperty('display'); } else {
            this.channelList.style.transition = 'width ' + this.transitionSpeed + 'ms';
            this.channelList.style.removeProperty('width');
            if (this.isDarkMatterLoaded) {
              this.settingsContainer.style.removeProperty('display');
      ...
      case 2: // I_MSG_BAR
        if (BdApi.getData('CollapsibleUI', 'msgBarButtonActive') === 'true') {
          ...
          if (this.disableTransitions) { this.msgBar.style.removeProperty('display'); } else {
            this.msgBar.style.maxHeight = this.msgBarMaxHeight + 'px';
            this.msgBar.style.removeProperty('overflow');
          }
          BdApi.setData('CollapsibleUI', 'msgBarButtonActive', 'true');
      ...
      case 3: // I_WINDOW_BAR
        if (BdApi.getData('CollapsibleUI', 'windowBarButtonActive') === 'true') {
          ...
          if (this.disableTransitions) { this.windowBar.style.removeProperty('display'); } else {
            if (this.isDarkMatterLoaded) {
              this.windowBar.style.height = '26px';
              this.windowBar.style.removeProperty('opacity');
            } else
      ...
      case 4: // I_MEMBERS_LIST
        if (BdApi.getData('CollapsibleUI', 'membersListButtonActive') === 'true') {
          ...
          if (this.disableTransitions) { this.membersList.style.removeProperty('display'); } else {
            this.membersList.style.maxWidth = this.membersListMaxWidth + 'px';
            this.membersList.style.removeProperty('min-width');
          }
          BdApi.setData('CollapsibleUI', 'membersListButtonActive', 'true');
      ...
      case 5: // I_USER_AREA
        if (BdApi.getData('CollapsibleUI', 'userAreaButtonActive') === 'true') {
          ...
          if (this.disableTransitions) { this.userArea.style.removeProperty('display'); } else { this.userArea.style.maxHeight = this.userAreaMaxHeight + 'px'; }
          BdApi.setData('CollapsibleUI', 'userAreaButtonActive', 'true');
          this.userAreaButton.classList.add(this.classSelected);
        }
        break;
      ...
      case 6: // I_CALL_CONTAINER
        if (BdApi.getData('CollapsibleUI', 'callContainerButtonActive') === 'true') {
          ...
          if (document.querySelector('.' + this.classCallContainer)) {
            if (this.disableTransitions) { document.querySelector('.' + this.classCallContainer).style.removeProperty('display'); } else {
              if (document.querySelector('.' + this.classNoChat))
                document.querySelector('.' + this.classCallContainer).style.maxHeight = BdApi.DOM.screenHeight + 'px';
              else
                document.querySelector('.' + this.classCallContainer).style.maxHeight = (BdApi.DOM.screenHeight - 222) + 'px';
      ...
      case 7: // I_USER_PROFILE
        if (BdApi.getData('CollapsibleUI', 'profilePanelButtonActive') === 'true') {
          ...
          if (this.disableTransitions) { this.profilePanel.style.removeProperty('display'); } else {
            this.profilePanel.style.maxWidth = this.profilePanelMaxWidth + 'px';
            this.profilePanel.style.removeProperty('min-width');
          }
          BdApi.setData('CollapsibleUI', 'profilePanelButtonActive', 'true');
