    ...
    return class CollapsibleUI extends Plugin {
        ...
        getSettingsPanel = () => {
            ...
            var settingServerList = new zps.Slider('Server List',
                                                   ...
                                                   null,
                                                   {markers:[0,1,2,3,4,5,6,7,8], stickToMarkers: true, equidistant: true});
        ...
        addButtonClickHandlers = () => {
            ...
            if (this.profilePanelButton) {
                ...
                this.profilePanelButton.addEventListener('mouseenter', function() {
                    this.tooltip = cui.createTooltip(cui.localeLabels.profilePanel + ` (${cui.keyStringList[cui.I_USER_PROFILE]})`, this);
                }, {signal: this.eventListenerSignal});
                    ...
                    window.addEventListener('resize', function(event) {
                        if (cui.serverListButton &&
                           ...
                           (((cui.isHSLLoaded ? BdApi.DOM.screenHeight : BdApi.DOM.screenWidth) < cui.autoCollapseThreshold[cui.I_SERVER_LIST] && BdApi.getData('CollapsibleUI', 'serverListButtonActive') === 'true') ||
                            ((cui.isHSLLoaded ? BdApi.DOM.screenHeight : BdApi.DOM.screenWidth) > cui.autoCollapseThreshold[cui.I_SERVER_LIST] && BdApi.getData('CollapsibleUI', 'serverListButtonActive') === 'false'))) {
                            cui.toggleButton(cui.I_SERVER_LIST);
                        }
                        ...
                        if (cui.profilePanelButton &&
                           ...
                           ((BdApi.DOM.screenWidth < cui.autoCollapseThreshold[cui.I_USER_PROFILE] && BdApi.getData('CollapsibleUI', 'profilePanelButtonActive') === 'true') ||
                            (BdApi.DOM.screenWidth > cui.autoCollapseThreshold[cui.I_USER_PROFILE] && BdApi.getData('CollapsibleUI', 'profilePanelButtonActive') === 'false'))) {
                            cui.toggleButton(cui.I_USER_PROFILE);
                        }
            ...
            for (let i = 1; i <= this.buttonsOrder.length; i++) { // lgtm[js/unused-index-variable]
                ...
                if (i == this.buttonsOrder[this.I_USER_PROFILE]) {
                    if (this.buttonsOrder[this.I_USER_PROFILE] && this.profilePanel) {
                        this.profilePanelButton = this.addToolbarIcon(this.localeLabels.profilePanel, '<path fill="currentColor" d="M11 6a3 3 0 1 1-6 0 3 3 0 0 1 6 0z"/><path fill="currentColor" fill-rule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1z"/>', '-1 -1 18 18');
                    } else {
            ...
            if (this.profilePanel) {
                if (this.buttonsOrder[this.I_USER_PROFILE] || this.disabledButtonsStayCollapsed) {
                    if (BdApi.getData('CollapsibleUI', 'profilePanelButtonActive') === 'false') {
                        if (this.profilePanelButton) this.profilePanelButton.classList.remove(this.classSelected);
                        if (this.disableTransitions) {
                        ...
                        if (this.profilePanelButton) this.profilePanelButton.classList.add(this.classSelected);
                    } else {
